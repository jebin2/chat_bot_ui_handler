...
def qwen_ui_chat(user_prompt, system_prompt=None, file_path=None, config=None):
	try:
		config = config or BrowserConfig()
		config.docker_name = "qwen_ui_chat"
		...
		with BrowserManager(config) as page:
			try:
				...
				input = page.locator('textarea[id="chat-input"]').first
				...
				page.wait_for_selector('#send-message-button', timeout=15000)
				page.wait_for_timeout(2000)

				result_text = page.locator('#response-message-body').last.inner_text()
				...
				logger_config.error(f"Error during qwen_ui_chat: {e} {traceback.format_exc()}")
				try:
					utils.save_screenshot(page)
				except:
					pass
