...
def copilot_ui_chat(user_prompt, system_prompt=None, file_path=None, config=None):
	try:
		config = config or BrowserConfig()
		config.docker_name = "copilot_ui_chat"
		...
		with BrowserManager(config) as page:
			try:
				...
				input = page.locator('textarea[id="userInput"]').first
				...
				send_button = page.locator('button[aria-label="Submit message"]').first
				send_button.click()
				logger_config.info("'Send' button clicked")
				page.wait_for_timeout(2000)
				utils.save_screenshot(page)

				logger_config.info("Waiting for results in 'button[aria-label=\"Talk to Copilot\"]' container...")
				page.wait_for_selector('button[aria-label="Talk to Copilot"]', timeout=15000)
				page.wait_for_timeout(2000)

				result_text = page.locator('div[data-content="ai-message"] p').last.inner_text()
				...
				logger_config.error(f"Error during copilot_ui_chat: {e} {traceback.format_exc()}")
				try:
					utils.save_screenshot(page)
				except:
					pass
