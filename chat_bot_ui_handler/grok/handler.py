...
def grok_ui_chat(user_prompt, system_prompt=None, file_path=None, config=None):
	try:
		config = config or BrowserConfig()
		config.docker_name = "grok_ui_chat"
		...
		with BrowserManager(config) as page:
			try:
				...
				input = page.locator('textarea[aria-label="Ask Grok anything"]').first
				...
				logger_config.info("Waiting for results in 'aria-label=\"Enter voice mode\"' container...")
				page.wait_for_selector('button[aria-label="Enter voice mode"]', timeout=15000)
				page.wait_for_timeout(2000)

				result_text = page.locator('#last-reply-container .message-bubble').last.inner_text()
				...
				logger_config.error(f"Error during grok_ui_chat: {e} {traceback.format_exc()}")
				try:
					utils.save_screenshot(page)
				except:
					pass
